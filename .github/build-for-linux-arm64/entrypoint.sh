...
if [ "$INPUT_TARGET" = "x86_64-unknown-linux-gnu" ]; then
    ...
    apt-get update
    apt-get install -y pkg-config libglib2.0-dev libncurses6 libtinfo6 linux-libc-dev libncursesw6 libssl3 libcups2
    apt-get install -y --no-install-recommends g++-aarch64-linux-gnu libc6-dev-arm64-cross libwebkit2gtk-4.1-dev libappindicator3-dev libgtk-3-dev patchelf librsvg2-dev libayatana-appindicator3-dev
